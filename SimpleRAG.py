...
# ==============================
# 5️⃣ Gọi Hugging Face API trực tiếp
# ==============================
from huggingface_hub import InferenceClient
import os
import traceback
...
def hf_infer(prompt: str) -> str:
    """
    Hàm gọi Hugging Face Inference API tương thích mọi version:
    - Tự động thử chat_completion với max_new_tokens / max_tokens / không giới hạn
    - Fallback sang chat() hoặc text_generation() nếu cần
    """
    model_id = "mistralai/Mistral-7B-Instruct-v0.3"
    token = os.getenv("HUGGINGFACEHUB_API_TOKEN")
    if not token:
        print("⚠️ Thiếu HUGGINGFACEHUB_API_TOKEN trong .env")
        return ""
    ...
    # --- 1️⃣ Thử chat_completion với nhiều cấu hình ---
    if hasattr(client, "chat_completion"):
        for kwargs in [
            dict(messages=messages, max_new_tokens=1024, temperature=0.3),
            dict(messages=messages, max_tokens=1024, temperature=0.3),
            dict(messages=messages, temperature=0.3)
        ]:
            try:
                print(f"ℹ️ Thử client.chat_completion(...) với kwargs={list(kwargs.keys())}")
                ...
                print(f"⚠️ Bỏ qua chat_completion cấu hình {list(kwargs.keys())}: {e}")
            except Exception as e:
                print("⚠️ Lỗi khi gọi chat_completion:", e)

    # --- 2️⃣ Thử chat() nếu có ---
    if hasattr(client, "chat"):
        try:
            print("ℹ️ Thử client.chat(...)")
            resp = client.chat(messages=messages, max_new_tokens=1024, temperature=0.3)
            return getattr(resp, "generated_text", str(resp))
        except Exception as e:
            print("⚠️ Lỗi khi gọi chat:", e)

    # --- 3️⃣ Thử text_generation() ---
    if hasattr(client, "text_generation"):
        try:
            print("ℹ️ Thử client.text_generation(...) (fallback cuối)")
            resp = client.text_generation(plain_prompt, max_new_tokens=1024, temperature=0.3)
            return getattr(resp, "generated_text", str(resp))
        except Exception as e:
            print("⚠️ Lỗi khi gọi text_generation:", e)
...
if __name__ == "__main__":
    ...
    Quy ước:
    - anomaly = -1 → bất thường
    - anomaly = 1 → bình thường
    - Một ngày có thể có nhiều khoảng bất thường liên tiếp, hãy gộp lại khi phân tích.
    - Với mỗi khoảng bất thường, nêu 1–3 trường KPI nổi bật nhất.

     Yêu cầu quan trọng:
    - Sau khi phân tích, hãy xuất kết quả dưới dạng JSON có cấu trúc:
    [
      {{
        "date": "<YYYY-MM-DD HH:MM:SS>",
        "node": <số node>,
        "field": "<tên trường KPI>",
        "reason": "<mô tả nguyên nhân>"
      }},
      ...
    ]
    ...
    remove_file_safe("../result_structured.json")
